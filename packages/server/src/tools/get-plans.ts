...
export async function handleGetPlans(
  ...
  const response = await client.getPaginated<Plan>(`get_plans/${params.project_id}`, queryParams);
...
export async function handleGetPlan(client: TestRailClient, params: { plan_id: number }) {
