    ...
    priorities.length === 0
      ...
      : `# Priorities (${priorities.length})\n\n` +
        priorities
          .map(
            (p) =>
              `- **[${p.id}] ${p.name}** (${p.short_name}) — Default: ${p.is_default ? 'Yes' : 'No'}`,
          )
          .join('\n');
  return {
  ...
  const text =
    caseTypes.length === 0
      ...
      : `# Case Types (${caseTypes.length})\n\n` +
        caseTypes
          .map((t) => `- **[${t.id}] ${t.name}** — Default: ${t.is_default ? 'Yes' : 'No'}`)
          .join('\n');
  return {
    content: [{ type: 'text' as const, text }],
    ...
    caseFields.length === 0
      ...
      : `# Case Fields (${caseFields.length})\n\n` +
        caseFields
          .map(
            (f) =>
              `- **${f.label}** (${f.system_name}) — Type: ${f.type_id}, Active: ${f.is_active ? 'Yes' : 'No'}`,
          )
          .join('\n');
  return {
    content: [{ type: 'text' as const, text }],
...
export async function handleGetTemplates(client: TestRailClient, params: { project_id: number }) {
...
export async function handleGetUsers(client: TestRailClient, params: { project_id?: number }) {
...
export async function handleGetSuites(client: TestRailClient, params: { project_id: number }) {
